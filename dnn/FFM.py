...
class trans_layer(nn.Module):
    ...
    def forward(self,x):
        ...
        # out_1 = self.relu(self.conv_l2_1(x_1)) + s_1 #feature_len * feature_num
        # out_2 = self.relu(self.conv_l2_2(x_2)) + s_2
        # out_3 = self.relu(self.conv_l2_3(x_3)) + s_3
...
class fusion_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # arm_emb = self.lrelu(self.conv_1(arm_emb)) #feature_len * feature_num
        # x86_emb = self.lrelu(self.conv_2(x86_emb))
        # bet_emb = self.lrelu(self.conv_3(bet_emb))
...
class Classifier_Net(nn.Module):#设置网络
    def __init__(self, feature_len,  kernel_size, n_output):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(feature_len*6, feature_len*12),#五个hidden state
            nn.BatchNorm1d(feature_len*12),
            nn.ReLU(inplace=True),
            nn.Linear(feature_len*12, feature_len*24),
            nn.BatchNorm1d(feature_len*24),
            nn.ReLU(inplace=True),
            nn.Linear(feature_len*24, feature_len*12),
            nn.BatchNorm1d(feature_len*12),
            nn.ReLU(inplace=True),
            nn.Linear(feature_len*12, feature_len*6),
            nn.BatchNorm1d(feature_len*6),
            nn.ReLU(inplace=True),
            # nn.Linear(feature_len*6, feature_len*3),
            # nn.BatchNorm1d(feature_len*3),
            # nn.ReLU(inplace=True),
            # nn.Linear(feature_len*3, feature_len),
            # nn.BatchNorm1d(feature_len),
            # nn.ReLU(inplace=True),
            nn.Linear(feature_len*6, n_output))
        ...
        self.classifier_shallow_concat = nn.Sequential(
            nn.Dropout(p=0.5),
            nn.Linear(feature_len*6, 48),
            nn.BatchNorm1d(48),
            nn.ReLU(inplace=True),
            nn.Linear(48, feature_len*6),
            nn.BatchNorm1d(feature_len*6),
            nn.ReLU(inplace=True),
            # nn.Dropout(p=0.5),
            nn.Linear(feature_len*6, feature_len*3),
    ...
    def forward(self, x):
        ...
        # #loop2
        # # code_1_emb = self.fusion_layer(code_1_emb)
        # # code_2_emb = self.fusion_layer(code_2_emb)
        # # print(code_2_emb[0].size())#[512, 1, 96]
        ...
        # code1_embedding = torch.cat ( (code_1_emb[0], code_1_emb[1], code_1_emb[2]), dim=2) + code_1
        # code2_embedding = torch.cat ( (code_2_emb[0], code_2_emb[1], code_2_emb[2]), dim=2) + code_2

        code_embedding = torch.cat ( (code1_embedding, code2_embedding), dim=1)#torch.Size([512, 576])

        # # print(code_embedding.size())

        # # output_embedding = self.classifier(code_embedding)
        # output_embedding = self.classifier(x)
        # output_embedding = self.classifier_shallow_concat(x)
